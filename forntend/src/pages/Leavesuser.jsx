...
const Leaves = () => {
  ...
  useEffect(() => {
    const fetchPresentEmployees = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/employees/present`);
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/Leave/leaves`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/Leave/request`, submissionData, {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/Leave/${id}/status`, { status });
      setLeaves(prev =>
              ...
              {filteredLeaves.map(leave => (
                <tr key={leave._id}>
                  ...
                  <td>
                    {leave.documentUrl ? (
                      <a
                        href={`${import.meta.env.VITE_BACKEND_URL}/uploads/${encodeURIComponent(leave.documentUrl.split('uploads/')[1])}`}
                        target="_blank"
