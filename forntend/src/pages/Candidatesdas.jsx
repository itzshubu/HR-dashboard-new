...
const Candidates = () => {
  ...
  const fetchCandidates = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/candidates/all`);
  ...
  const addCandidate = async (candidateData) => {
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/candidates/add`, candidateData);
      fetchCandidates();
    } catch (error) {
  ...
  const updateCandidateStatus = async (id, newStatus) => {
    try {
      await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/candidates/status/${id}`, {
        status: newStatus,
      });
      fetchCandidates();
  ...
  const addEmploye = async (candidateId) => {
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/employees/add`, { id: candidateId });
      fetchEmployes();
    } catch (error) {
